...
$pdf->SetAuthor("etwinning Greece");
...
$LOGO_FILE = $CFG->wwwroot.'/mod/certificate/pix/' .CERT_IMAGE_SIGNATURE. '/logo_moodle.jpg';
$SIGN_FILE = $CFG->wwwroot.'/mod/certificate/pix/' .CERT_IMAGE_SIGNATURE. '/sign_ntzimop.png';
$SIGN1_FILE = $CFG->wwwroot.'/mod/certificate/pix/' .CERT_IMAGE_SIGNATURE. '/'. $course_info->sign_file1;
...
$filename = $path . '/cert_moodle_21_'. strtolower($role). '.html';

//$filename = $path . '/cert_moodle_19_student.html';
// Remove this line to show the certification
//$filename = $path . '/cert_na.html';
...
$statement = str_replace("%SIGN1_FILE%", $SIGN1_FILE,$statement);
//$statement = str_replace("%FULLNAME%", mb_strtoupper(trim(fullname($USER)), 'UTF-8') ,$statement);
$statement = str_replace("%FONT_SIZE%", $FONT_SIZE, $statement);
$statement = str_replace("%FOREAS_TITLE%", $course_info->foreas_title, $statement);
...
$statement = str_replace("%COURSE_LENGTH%", $course_info->duration . ' ώρες' ,$statement);
$statement = str_replace("%WEEKS%", '12' ,$statement);
$statement = str_replace("%PROTOKOLO_2014_15%", $course_info->protokolo ,$statement);
...
$statement = str_replace("%SIGN1_NAME%", $course_info->sign_name1 ,$statement);
$statement = str_replace("%SIGN1_TITLE%", $course_info->sign_title1 ,$statement);


//echo $course_info->course_year . "</br>";
...
$sfragida = "$CFG->dirroot/mod/certificate/pix/signatures/" .  $course_info->sfragida_file;
if ($role == "teacher") {
	$sfragidaposX = 160;
	$sfragidaposY = 150;
} else {
